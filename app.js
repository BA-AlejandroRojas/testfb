...
function exportTicketAsImage(node, id){
  ...
  const responseCard = el('.response-card', node);
  btn.classList.add('exporting');

  const prevBtnDisplay = btn.style.display;
  const prevRemoveDisplay = removeBtn.style.display;
  const prevResponseDisplay = responseCard ? responseCard.style.display : null;
  ...
  const restore = () => {
    ...
    if(responseCard) responseCard.style.display = prevResponseDisplay;
    btn.classList.remove('exporting');
  };

  const theme = document.documentElement.getAttribute('data-theme') === 'light' ? '#F3F2F8' : '#1A1E26';

  html2canvas(node, { backgroundColor: theme, scale: 3, useCORS: true, logging: false }).then(canvas => {
    restore();
